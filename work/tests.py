        ...
        for field_item in fields:
            if field_item["is_null"]:
                self.check_integer_field(
                    field_item["field"], field_item["verbose"], True)
            else:
                self.check_integer_field(
                    field_item["field"], field_item["verbose"], False, -1)
...
class TestView(TestCase):
    ...
    def test_check_sheet_title(self):
        ...
        response = self.client.get(
            reverse('sheets_title_check'), {
                'id': self.sheet.id, 'title': self.sheet.title}, **auth_header)
        self.assertEqual(response.status_code, 200)
